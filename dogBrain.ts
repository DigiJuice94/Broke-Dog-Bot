...
class DogBrain {
  ...
  markDecision(c:Candidate,decision:"REJECTED"|"BOUGHT"){if(!config.dogBrainEnabled)return;this.observe(c);const r=this.state.records.find(x=>x.mint===c.token.address&&!x.resolved);if(!r)return;r.decision=decision;r.mode=config.liveTrading?"LIVE":"PAPER";r.decisionAt=Date.now();r.decisionReason=c.decisionReason;r.lane=this.lane(c);r.score=c.score;r.confidence=c.dataConfidence;r.features=this.vector(c);r.sourceFirstSeenAt=Object.fromEntries(Object.entries(c.sourceFirstSeenAt??{}).map(([k,v])=>[k,Number(v)]));r.firstBuyScoreAt=c.firstBuyScoreAt;const snap=c.snapshots.at(-1);r.missingFields=[snap?.routeQuality==null?"routeQuality":null,snap?.onChainRisk==null?"onChainRisk":null,snap?.onChainRisk?.top10Pct==null&&snap?.top10HolderPct==null?"top10HolderPct":null,snap?.bundleStatus==="unknown"||snap?.bundleStatus==="skipped"?"bundleSignal":null].filter(Boolean) as string[];r.decisionSnapshot={at:Date.now(),ageSec:Math.max(0,Math.round((Date.now()-c.firstSeenAt)/1000)),firstThresholdDelaySec:c.firstBuyScoreAt?Math.max(0,Math.round((c.firstBuyScoreAt-c.firstSeenAt)/1000)):null,entryDelayAfterThresholdSec:c.firstBuyScoreAt?Math.max(0,Math.round((Date.now()-c.firstBuyScoreAt)/1000)):null,sources:[...c.sources],sourceFirstSeenAt:r.sourceFirstSeenAt,route:{quality:snap?.routeQuality,quotedAt:snap?.routeQuotedAt,roundTripPct:snap?.routeRoundTripPct,router:snap?.routeRouter,mode:snap?.routeMode,feeBps:snap?.routeFeeBps,buy:snap?.buyRoute,sell:snap?.sellRoute},safety:{bundleStatus:snap?.bundleStatus,bundleRisk:snap?.bundleRisk,onChainRisk:snap?.onChainRisk,safetyElapsedMs:snap?.onChainRisk?.elapsedMs},momentum:{price1m:snap?.priceChange1mPct,price5m:snap?.priceChange5mPct,buys1m:snap?.buys1m,sells1m:snap?.sells1m,volume1mUsd:snap?.volume1mUsd,volume5mUsd:snap?.volume5mUsd}};this.save();}
