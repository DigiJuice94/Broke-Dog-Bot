# Broke Dog Bot v1.5.2 — Winning Baseline + Persistent Dog Brain Memory

**Only two intentional changes from the preserved winning baseline:**
- Paper sizing caps remain **NORMAL $45 / ELITE $55 / FLAME $70**.
- Dog Brain memory is hardened so learned records/weights can survive Railway updates when `DOG_BRAIN_FILE` points to a persistent Railway Volume. Saves are atomic and keep a `.bak` recovery copy; startup prints **MEMORY REMEMBERED** when prior learning is found.

**No entry scoring, safety, discovery, buy thresholds, exit logic, or Dog Brain learning rules were changed.**
...
- Paper sizing stays controlled: **NORMAL max $45, ELITE max $55, FLAME max $70** by default.
...
Important: Railway's normal filesystem may not survive redeploys. Mount a Railway persistent volume (recommended mount path: `/data`) and point `STATE_FILE`, `PAPER_LEDGER_FILE`, and `DOG_BRAIN_FILE` into that mounted path. For Dog Brain specifically, set `DOG_BRAIN_FILE=/data/broke-dog-brain-v1.json`. On boot, `🧠💾 MEMORY REMEMBERED` confirms prior learning was found and loaded. Dog Brain also maintains `broke-dog-brain-v1.json.bak` beside the main file for recovery if the primary file becomes unreadable.
