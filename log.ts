...
export const log={
  ...
  scan(data:{name:string;symbol:string;priceUsd?:number;score:number;confidence:number;status:string;reason?:string;sources?:string[];rankText?:string;details?:{
    ...
    top10HolderPct?:number;top1HolderPct?:number;top5HolderPct?:number;bundleClass?:string;devRisk?:string;holderRisk?:string;linkedSupply?:number;socialScore?:number;socialAccounts?:string;meta?:string;smart?:string;metaRunner?:boolean;deep?:string}}){
    ...
    const detail=d?` | B/S(${flowWindow}):${buys??"?"}/${sells??"?"} Vol:${vol==null?"?":`$${Math.round(vol)}`} Liq:${d.liquidityUsd==null?"?":`$${Math.round(d.liquidityUsd)}`} Holders:${d.holderCount??"?"}${safety?` ${safety}`:` Top10:${d.top10HolderPct==null?"?":`${d.top10HolderPct.toFixed(1)}%`}`}${d.deep?` Deep:${d.deep}`:""}${social}${smart}`:"";
